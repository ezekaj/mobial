...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!config) {
      return errorResponse('No 2FA setup in progress. Please start the setup first.', 400);
    }
    
    const setupData = JSON.parse(config.value) as { secret: string; createdAt: string };
    ...
    if (Date.now() - setupTime.getTime() > 10 * 60 * 1000) {
      ...
      return errorResponse('2FA setup has expired. Please start the setup again.', 400);
    }
