...
import { createReview, getApprovedReviews, getReviewStats } from "@/services/review-service"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderId, name, email, rating, title, text, destination, countryCode } = body

    // Validation
    if (!name || !email || !rating || !title || !text) {
      return errorResponse("Name, email, rating, title, and text are required", 400)
    }
