"use client"

import { useState, useEffect } from "react"
import { MessageCircle, X } from "lucide-react"
import { cn } from "@/lib/utils"

const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || ""
const CRISP_WEBSITE_ID = process.env.NEXT_PUBLIC_CRISP_WEBSITE_ID || ""
const SUPPORT_MESSAGE = "Hi! I need help with my MobiaL eSIM."

declare global {
  interface Window {
    $crisp?: unknown[]
    CRISP_WEBSITE_ID?: string
  }
}

function useCrisp() {
  const [loaded, setLoaded] = useState(false)

  useEffect(() => {
    if (!CRISP_WEBSITE_ID) return

    // Initialize Crisp
    window.$crisp = window.$crisp || []
    window.CRISP_WEBSITE_ID = CRISP_WEBSITE_ID

    // Hide Crisp's default button — we use our own FAB
    window.$crisp.push(["safe", true])
    window.$crisp.push(["config", "hide:on:away", true])

    const script = document.createElement("script")
    script.src = "https://client.crisp.chat/l.js"
    script.async = true
    script.onload = () => {
      // Hide the default Crisp chat button after it loads
      window.$crisp?.push(["do", "chat:hide"])
      setLoaded(true)
    }
    document.head.appendChild(script)

    return () => {
      // Cleanup is not strictly needed as Crisp persists, but keep ref clean
    }
  }, [])

  const openChat = () => {
    if (window.$crisp) {
      window.$crisp.push(["do", "chat:show"])
      window.$crisp.push(["do", "chat:open"])
    }
  }

  return { loaded, openChat }
}

export function ChatWidget() {
  const [isOpen, setIsOpen] = useState(false)
  const crisp = useCrisp()

  const hasCrisp = !!CRISP_WEBSITE_ID
  const hasWhatsApp = !!WHATSAPP_NUMBER

  // Nothing to show if no chat channels configured
  if (!hasCrisp && !hasWhatsApp) return null

  const whatsappUrl = hasWhatsApp
    ? `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(SUPPORT_MESSAGE)}`
    : ""

  const handleFabClick = () => {
    // If Crisp is the only channel, open it directly
    if (hasCrisp && !hasWhatsApp) {
      crisp.openChat()
      return
    }
    // Otherwise show the menu
    setIsOpen(!isOpen)
  }

  return (
    <div className="fixed bottom-6 right-6 z-40">
      {/* Expanded options */}
      {isOpen && (
        <div className="mb-3 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden w-72 animate-in fade-in slide-in-from-bottom-3 duration-200">
          <div className="p-4 border-b border-border/50 bg-muted/30">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-sm">Need help?</h3>
                <p className="text-xs text-muted-foreground">
                  We typically reply in minutes
                </p>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="p-1 hover:bg-muted rounded-lg"
                aria-label="Close chat menu"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
          </div>
          <div className="p-3">
            {/* Crisp live chat option */}
            {hasCrisp && (
              <button
                onClick={() => {
                  crisp.openChat()
                  setIsOpen(false)
                }}
                className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors group text-left"
              >
                <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center shrink-0">
                  <MessageCircle className="w-5 h-5 text-white" />
                </div>
                <div>
                  <p className="font-semibold text-sm group-hover:text-primary transition-colors">
                    Live Chat
                  </p>
                  <p className="text-xs text-muted-foreground">Chat with us now</p>
                </div>
              </button>
            )}

            {/* WhatsApp option */}
            {hasWhatsApp && (
              <a
                href={whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors group"
              >
                <div className="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center shrink-0">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                </div>
                <div>
                  <p className="font-semibold text-sm group-hover:text-primary transition-colors">
                    WhatsApp Support
                  </p>
                  <p className="text-xs text-muted-foreground">Chat with us</p>
                </div>
              </a>
            )}

            {/* Email option */}
            <a
              href="mailto:support@mobialo.eu"
              className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors group"
            >
              <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center shrink-0">
                <MessageCircle className="w-5 h-5 text-primary-foreground" />
              </div>
              <div>
                <p className="font-semibold text-sm group-hover:text-primary transition-colors">
                  Email Support
                </p>
                <p className="text-xs text-muted-foreground">
                  support@mobialo.eu
                </p>
              </div>
            </a>
          </div>
        </div>
      )}

      {/* FAB button */}
      <button
        onClick={handleFabClick}
        className={cn(
          "w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95",
          isOpen
            ? "bg-muted text-foreground"
            : "bg-primary text-primary-foreground"
        )}
        aria-label={isOpen ? "Close chat" : "Open chat"}
      >
        {isOpen ? (
          <X className="h-6 w-6" />
        ) : (
          <MessageCircle className="h-6 w-6" />
        )}
      </button>
    </div>
  )
}
